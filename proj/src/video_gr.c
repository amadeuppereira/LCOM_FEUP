...
static uint16_t *video_mem; /* Process (virtual) address to which VRAM is mapped */ ///< video_mem pointer
static uint16_t *double_buffer; ///< double buffer pointer
...
int vg_draw_xpm(char *xpm[] ,unsigned short x ,unsigned short y){
	...
	for (i = 0; i < width; ++i) {
		for (j = 0; j < height; ++j) {
			vg_set_color_pixel(x + i, y + j, *(pixmap + (i + j * width) * (bits_per_pixel / 8)));
		}
	}
	return 0;
}
...
uint16_t rgb(int r, int g, int b) {
	...
	c |= (r * (1 << modeInfo.RedMaskSize) / RGB_MAX) << modeInfo.RedFieldPosition;
	c |= (g * (1 << modeInfo.GreenMaskSize) / RGB_MAX) << modeInfo.GreenFieldPosition;
	c |= (b * (1 << modeInfo.BlueMaskSize) / RGB_MAX) << modeInfo.BlueFieldPosition;
