...
int kbd_read_scancode(unsigned long *scancode)
{
	...
	if (sys_inb(KBC_OUT_BUF ,scancode) != 0){  // reads the scancode from out buffer
...
int kbd_read_command(unsigned long *code) {
	...
	if (sys_outb(KBC_IN_BUF, READ_KBD_COMMAND) != 0) {  // writes command (0x20) in kbd in buffer (0x64)
		printf("Error calling sys_outb!\n");
		return 1;
	}

	if (sys_inb(KBC_OUT_BUF, code) != 0) {  // reads kbd out buffer (0x60)
...
int kbd_wrt_command(unsigned long code) {
	...
	if (sys_outb(KBC_IN_BUF, WRITE_KBD_COMMAND) != 0){ // writes command (0x60) in kbd in buffer (0x64)
		printf("Error calling sys_outb!\n");
		return 1;
	}
